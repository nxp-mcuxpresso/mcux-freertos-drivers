...
static void SMARTCARD_RTOS_Callback(void *handle, void *param)
{
    ...
    if (IS_IRQ_CONTEXT())
    {
        (void)xSemaphoreGiveFromISR(ctx->x_event, &reschedule);
        portYIELD_FROM_ISR(reschedule);
    }
    else
    {
        (void)xSemaphoreGive(ctx->x_event);
    }
}
...
{
    if ((NULL == ctx) || (0U == sourceClockHz) || (NULL == base))
    {
...
int SMARTCARD_RTOS_Transfer(rtos_smartcard_context_t *ctx, smartcard_xfer_t *xfer)
{
    ...
    if (kStatus_Success == retval)
    {
        ...
        (void)xSemaphoreTake(ctx->x_event, portMAX_DELAY);
    }
